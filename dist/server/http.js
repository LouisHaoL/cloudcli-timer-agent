...
import { intervalNextMs, isIntervalRule, isOneShotRule, isSchedulable, isValidCron, nextRunAtMs, resumeNextMs, scheduleNextMs, } from '../shared/schedule.js';
...
async function create(response, store, body) {
    ...
    if (runAt !== undefined)
        input.runAt = runAt; // parsed (an ISO string is not usable as an instant)
    else
        delete input.runAt; // cron/interval take precedence; a stray runAt is ignored
    if (!inbox)
        delete input.inbox;
...
async function patch(response, store, id, body) {
    ...
    const updated = await store.mutate(jobs => {
        ...
        if (body.skipNext === true) {
            ...
            if (isOneShotRule(job.schedule))
                throw new ApiError(400, 'a one-time job has no next occurrence to skip');
        ...
        if (body.nextRunAt !== undefined) {
            ...
            if (pinned === undefined)
                throw new ApiError(400, 'invalid nextRunAt (expected a ms epoch number or ISO datetime string)');
            if (job.schedule === undefined)
                throw new ApiError(400, 'job has no schedule to pin a next run to');
            if ((job.schedule.cron ?? '') !== '')
                throw new ApiError(400, 'cron jobs own their next run; edit the cron expression instead');
        ...
        if (typeof body.enabled === 'boolean') {
            ...
            if (body.enabled) {
                ...
                if (armed === undefined)
                    throw new ApiError(400, 'cannot compute the next run (invalid cron expression?)');
        ...
        case 'resume': {
            ...
            const updated = await store.mutate(jobs => {
                ...
                const armed = enabled
                    ...
                    : undefined;
                if (enabled && armed === undefined)
                    throw new ApiError(400, 'cannot compute the next run (invalid cron expression?)');
                const next = withSchedule(withStatus(job, job.status === 'archived' ? 'idle' : job.status, now), { enabled, ...(enabled ? { nextRunAt: armed } : {}) }, now);
