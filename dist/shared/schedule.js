...
export function parseCron(expr) {
    ...
    for (let index = 0; index < 5; index++) {
        ...
        const set = new Set();
        if (!parseField(fields[index], min, max, set))
            return null;
        sets.push(set);
    }
    const weekdays = new Set();
    for (const day of sets[4])
        weekdays.add(day === 7 ? 0 : day);
    return {
        minutes: sets[0],
        hours: sets[1],
        days: sets[2],
 ...
 * minutes, and agent-driven self-rescheduling needs a live AI tool.
 */
export function isIntervalRule(rule) {
 ...
 * stay unschedulable.
 */
export function isSchedulable(rule) {
    return rule !== null && rule !== undefined &&
 ...
 */
export function isOneShotRule(rule) {
 ...
 */
export function resumeNextMs(rule, baseMs, nowMs) {
    if (isIntervalRule(rule))
        return intervalNextMs(baseMs, rule.intervalMinutes, nowMs);
    if (isOneShotRule(rule))
        return rule.nextRunAt;
    const fromBase = nextRunAtMs(rule.cron, baseMs ?? nowMs);
    if (fromBase !== undefined && fromBase > nowMs)
        return fromBase;
    // The base's next slot already passed (server down / paused too long): skip
...
export function scheduleNextMs(rule, fromMs) {
    if (isIntervalRule(rule))
 ...
 * A missed stretch (server down, job paused, long run) stacks whole intervals
 ...
 */
export function intervalNextMs(anchorMs, intervalMinutes, fromMs) {
    const step = intervalMinutes * 60_000;
    let next = (anchorMs ?? fromMs) + step;
    if (next <= fromMs)
        next += Math.ceil((fromMs - next) / step) * step;
    return next;
}
/**
