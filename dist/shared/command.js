...
export function splitCommandArgs(input) {
    ...
    while (index < input.length) {
        ...
        if (char === '"') {
            ...
            if (!closed)
                throw new Error(`unterminated double quote in args: ${input}`);
            continue;
        }
        if (char === "'") {
            hasCurrent = true;
            ...
            if (close === -1)
                throw new Error(`unterminated single quote in args: ${input}`);
 ...
 */
export function truncateOutputTail(text, maxChars = OUTPUT_TAIL_CHARS) {
...
export function appendCapped(buffer, chunk) {
