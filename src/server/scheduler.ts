 ...
 */
import {
  intervalNextMs, isIntervalRule, isOneShotRule, isSchedulable, nextRunAtMs, scheduleNextMs,
...
function ensureNextRunAt(job: JobRecord, now: number): JobRecord {
  ...
  if (isOneShotRule(schedule)) return job
  ...
  const dueJobs = await store.mutate(jobs => {
    ...
    if (policy.enabled) {
      ...
      while (capacity > 0) {
        ...
        if (index !== -1) next[index] = dispatched
        else next.push(dispatched)
