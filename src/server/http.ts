...
let app: HttpAppOptions | undefined

/** Load the server CLI profile (falling back to the default). */
export async function loadProfile(): Promise<ServerProfile> {
  try {
    const raw = JSON.parse(await readFile(defaultProfileFile(), 'utf8')) as Partial<ServerProfile>
    if (typeof raw.command !== 'string' || raw.command === '') return { ...DEFAULT_PROFILE }
    return {
      command: raw.command,
...
async function saveProfile(profile: ServerProfile): Promise<void> {
...
function checkCron(cron: unknown): string | undefined {
  if (cron === undefined) return undefined
  if (typeof cron !== 'string') throw new ApiError(400, 'cron must be a string')
...
const publicJob = (job: JobRecord): JobRecord => job

/** Build and start the loopback HTTP server (random port; host handshake). */
export function startHttpServer(options: HttpAppOptions): Promise<Server> {
  app = options
...
async function handle(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (jobsMatch !== null) {
    ...
    if (method === 'POST' && id === '' && action === undefined) {
      const body = (await readBody(request)) as Partial<NewJobInput> | undefined
    ...
    if (method === 'DELETE' && id !== '' && action === undefined) {
      ...
      if (removed === undefined) return send(response, 404, { error: `job not found: ${id}` })
  ...
  if (path === '/v1/profile') {
    if (method === 'GET') return send(response, 200, { profile: await loadProfile() })
    if (method === 'PUT') {
      const body = (await readBody(request)) as Partial<ServerProfile> | undefined
...
async function create(response: ServerResponse, store: JobStore, body: Partial<NewJobInput> | undefined): Promise<void> {
...
async function patch(response: ServerResponse, store: JobStore, id: string, body: Record<string, unknown> | undefined): Promise<void> {
  if (body === undefined) throw new ApiError(400, 'body required')
  const now = Date.now()
  const updated = await store.mutate(jobs => {
    ...
    if (typeof body.title === 'string' && body.title.trim() !== '') job = { ...job, title: body.title.trim() }
    if (typeof body.description === 'string') job = { ...job, description: body.description.trim() }
    if (typeof body.prompt === 'string') job = { ...job, prompt: body.prompt.trim() }
    if (typeof body.command === 'string') job = { ...job, command: body.command.trim() }
    if (typeof body.args === 'string') job = { ...job, args: body.args.trim() }
    if (typeof body.workdir === 'string') job = { ...job, workdir: body.workdir.trim() || undefined }
    ...
    if (typeof body.session === 'string') job = { ...job, session: body.session.trim() || undefined }
    if (typeof body.timeoutMs === 'number') job = { ...job, timeoutMs: body.timeoutMs > 0 ? Math.round(body.timeoutMs) : undefined }
    if (typeof body.model === 'string') job = { ...job, model: body.model.trim() || undefined }
    if (typeof body.effort === 'string') job = { ...job, effort: body.effort.trim() || undefined }
    if (typeof body.priority === 'number') job = { ...job, priority: normalizePriority(body.priority) }
    if (typeof body.difficulty === 'number') job = { ...job, difficulty: normalizeDifficulty(body.difficulty) }
    if (typeof body.targetProject === 'string') job = { ...job, targetProject: body.targetProject.trim() || undefined }
    if (typeof body.inbox === 'boolean') {
      if (body.inbox) job = { ...job, inbox: true }
      else {
    ...
    if (body.kind === 'command' || body.kind === 'agent') {
      if (body.kind === 'command' && (job.command ?? '').trim() === '') throw new ApiError(400, 'command is required for command jobs')
      if (body.kind === 'agent' && job.prompt.trim() === '') throw new ApiError(400, 'prompt is required for agent jobs')
    ...
    if (body.cli !== undefined && typeof body.cli === 'object' && body.cli !== null) {
      const profile = body.cli as Record<string, unknown>
...
async function runAction(response: ServerResponse, store: JobStore, tick: () => Promise<void>, id: string, action: string): Promise<void> {
  const now = Date.now()
  switch (action) {
    case 'pause':
    case 'resume': {
    ...
    case 'run-now': {
      const stamped = await store.mutate(jobs => {
        ...
        if (index === -1) return undefined
        const job = jobs[index]!
        if (job.status === 'archived') throw new ApiError(400, 'job is archived; restart it first')
        if (job.kind !== 'command' && job.prompt.trim() === '') throw new ApiError(400, 'job has no prompt')
        return { jobs: jobs.map((item, at) => (at === index ? { ...job, runRequestedAt: now, updatedAt: now } : item)), result: true }
      })
      if (stamped === undefined) throw new ApiError(404, `job not found: ${id}`)
